...
export default {
  ...
  computed: {
    currentSong() {
      return this.musicinfo && this.musicinfo[this.playlistIndex];
    },
    audioPlayer() {
  ...
  methods: {
    ...
    updateAudio() {
      ...
      this.audioPlayer.volume = this.volume / 100;
      this.isPlaying = true;
      // play() 返回 Promise，被浏览器自动播放策略或资源加载失败拒绝时不抛异常
      const playPromise = this.audioPlayer.play();
      if (playPromise && typeof playPromise.catch === 'function') {
        playPromise.catch((err) => {
          console.warn('音频播放失败，将自动切换下一首:', err.message || err);
          this.isPlaying = false;
        });
      }
    },
    // 音频加载/播放出错时的兜底处理：自动跳过到下一首，避免播放器卡死
    onAudioError() {
      if (!this.musicinfo || this.musicinfo.length === 0) return;
      console.warn('音频资源加载失败，自动切换下一首');
      this.isPlaying = false;
      // 延迟切换，避免连续错误时快速循环
      setTimeout(() => {
        this.nextTrack();
    ...
    verifyAdminPassword() {
      // 优先使用环境变量中的密码，其次使用配置文件中的密码，避免明文硬编码
      const correctPassword = import.meta.env.VITE_ADMIN_PASSWORD || this.configdata.adminPassword || 'admin';
    ...
    changeAdminPassword() {
      ...
      // 验证当前密码（优先环境变量）
      const currentCorrectPassword = import.meta.env.VITE_ADMIN_PASSWORD || this.configdata.adminPassword || 'admin';
      if (this.currentPassword.trim() !== currentCorrectPassword) {
        this.passwordChangeError = '当前密码错误';
        return;
      }

      // 若密码通过环境变量设置，则不允许在前端修改
      if (import.meta.env.VITE_ADMIN_PASSWORD) {
        this.passwordChangeError = '密码已通过环境变量配置，无法在页面修改';
        return;
      }
