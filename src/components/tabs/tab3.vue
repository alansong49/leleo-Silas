...
export default {
  ...
  methods: {
    ...
    // 滚动到当前播放的歌曲
    scrollToCurrentSong() {
      const songItems = this.$refs.songItems;
      if (Array.isArray(songItems) && songItems[this.currentIndex]) {
        const currentSongItem = songItems[this.currentIndex];
        if (currentSongItem && currentSongItem.$el) {
